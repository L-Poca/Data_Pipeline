...
def plot_training_curves(
    ...
    for idx, metric in enumerate(metrics_to_plot):
        ...
        ax.plot(epochs, combined_history[metric], 
                label=f'Train {metric}', linewidth=2, marker='o', markersize=3)
        
        # Plot validation if exists
        val_metric = f'val_{metric}'
        if val_metric in combined_history:
            ax.plot(epochs, combined_history[val_metric], 
                   label=f'Val {metric}', linewidth=2, marker='s', markersize=3)
        
        # Add vertical line for fine-tuning start
        if epochs_fe is not None:
            ax.axvline(x=epochs_fe, color='red', linestyle='--', 
                      linewidth=2, label='Fine-tuning start', alpha=0.7)
